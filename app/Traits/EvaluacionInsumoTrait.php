...
trait EvaluacionInsumoTrait
{
    ...
    {
        $detalle = "Monto asignado a la Solicitud N° {$solicitud->id} de Insumos y Servicios";
    ...
    public function setHistorialContrato(Solicitud $solicitud, $id, $monto)
    {
        $detalle = "Monto asignado a la Solicitud N° {$solicitud->id} de Insumos y Servicios";
        $historial    = new HistorialContrato(['contrato_suministro_id' => $id, 'periodo_id' => lastPeriodo($id), 'detalle' => $detalle, 'cantidad' =>  $monto, 'type' => 'egreso']);
        $solicitud->historialesContrato()->save($historial);
        syncMontoContrato($id, $monto, 'egreso');
    }
